 ...
 * @brief Get temperature, gyroscope and accelerometer data by FIFO
...
void setup() {
  ...
  */
  ...
  ICM42688.setODRAndFSR(/* who= */ACCEL,/* ODR= */ODR_1KHZ, /* FSR = */FSR_0);
  /**
  ...
  ICM42688.startFIFOMode();  //Enable FIFO
}

void loop() {
  float accelDataX,accelDataY,accelDataZ,gyroDataX,gyroDataY,gyroDataZ,tempData;

  ICM42688.getFIFOData(); //Read FIFO data, read temperature, gyroscope and accelerometer data and save them for parsing.
  tempData= ICM42688.getTemperature(); //Get measured temperature value, unit: ℃
